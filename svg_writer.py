...
def writeSVG(fileName, iconGraphics, transformation = None, coOrdinateSystem = None):
  elementId = 0
  width = 100
  height = 100
  ...
  if transformation:
    ...
    scale = iconGraphics[0]['coordinateSystem']['initialScale']
    width = width * scale
    height = height * scale
    svg = svgutils.transform.fromfile(fileName)
    originalSVG = svgutils.compose.SVG(fileName)
    originalSVG.scale_xy(scale, scale)
